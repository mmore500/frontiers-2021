...
Hierarchical levels are introduced into the system through multiple separate, but overlaid, instantiations of this hereditary grouping scheme.
We refer to each independent hereditary grouping system as a ``level.''
In some experimental treatments, we allowed two levels of hereditary grouping, identified here as L0 and L1.
On L0, resource waves extended a radius of two toroidal tiles.
On L1 they extended a radius of six toroidal tiles.
...
Due to the different radii of resource waves on different levels, level one selects for small hereditary groups and level two selects for larger hereditary groups.

Each cell contained a pair of separate hereditary group IDs, the first for level one and the second for level two.
We kept these hereditary group IDs hierarchically nested by constraining inheritance during reproduction.
...
\end{enumerate}
However, daughter cells could not inherit just the L1 hereditary group ID.
...
Figure \ref{fig:morphology-wt} depicts hierarchically-nested hereditary groupings assumed by an evolved strain.

An evolutionary transition in individuality can readily be evaluated within the DISHTINY framework with respect to hereditary groups.
In addition to a potentially functionally cooperative relationship, shared hereditary group IDs --- which may only systematically arise through inheritance --- imply a close hereditary relationship.
Because new hereditary group IDs arise first in a single cell, hereditary groups are reproductively bottlenecked analogously to a "Staying Together" life cycle (rather than a "Coming Together" life cycle) \cite{staps2019emergence}.
This precludes chimeric groups, except for mutations arising from somatic reproduction and rare cases of hereditary group ID collision.
...
Further, we can screen for the evolution of complex multicellularity by assessing cell-cell messaging, regulatory patterning, and functional differentiation between cells within hereditary groups \cite{knoll2011multiple}.

\subsection{Channel Group Life Cycle} \label{sup:channel_group_life_cycle}

Mature hereditary groups enjoy a considerable advantage over fledging propagules.
...
Figure \ref{fig:intermessaging-intergroup_border-phen} compares the cell-cell messaging, resource sharing, and parent-propagule phenotypes between wild type and cell-cell messaging knockout variants of a strain evolved under the Nested-Wave treatment.
